 ...
 */
public class HiTSDBHAClient {
    private static final Logger logger = LoggerFactory.getLogger(HiTSDBHAClient.class);

    private HiTSDB tsdbMaster, tsdbSlave;

    public HiTSDBHAClient(HiTSDBConfig master, HiTSDBConfig slave) {
        tsdbMaster = HiTSDBClientFactory.connect(master);
        tsdbSlave = HiTSDBClientFactory.connect(slave);
    }
