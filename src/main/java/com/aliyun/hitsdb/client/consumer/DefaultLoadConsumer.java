...
public class DefaultLoadConsumer implements Consumer {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultBatchPutConsumer.class);
    private CompressionBatchPointsQueue dataQueue;
    ...
    public DefaultLoadConsumer(CompressionBatchPointsQueue buffer, HttpClient httpclient, RateLimiter rateLimiter, HiTSDBConfig config) {
    ...
    public void start() {
        for (int i = 0; i < batchPutConsumerThreadCount; i++) {
            threadPool.submit(new LoadRunnable(this.dataQueue, this.httpclient, this.config,this.countDownLatch,this.rateLimiter));
        }
    }
